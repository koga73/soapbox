...
namespace eosio
{
	 ...
	 */
	class [[eosio::contract("soapboxtoken")]] token : public contract
	{
	public:
		...
		ACTION init(float wax_to_soap_rate);
		using init_action = action_wrapper<name("init"), &token::init>;

#if TESTNET
		[[eosio::on_notify("soapboxmain::withdraw")]] void deposit(name &alias, asset &quantity);
		[[eosio::on_notify("eosio.token::transfer")]] void deposit_wax(const name &from,
																	   const name &to,
																	   const asset &quantity,
																	   const string &memo);
#else
		[[eosio::on_notify("soapboxb.gm::withdraw")]] void deposit(name &alias, asset &quantity);
		[[eosio::on_notify("eosio.token::transfer")]] void deposit_wax(const name &from,
																	   const name &to,
																	   const asset &quantity,
																	   const string &memo);
